...
package com.example.checkMigrationFlagAuthNode;
...
@Node.Metadata(outcomeProvider  = AbstractDecisionNode.OutcomeProvider.class,
               configClass      = MigrationFlagAuthNode.Config.class)
public class MigrationFlagAuthNode extends AbstractDecisionNode {

    private final Pattern DN_PATTERN = Pattern.compile("^[a-zA-Z0-9]=([^,]+),");
    private final Logger logger = LoggerFactory.getLogger(MigrationFlagAuthNode.class);
    ...
    @Inject
    public MigrationFlagAuthNode(@Assisted Config config, @Assisted Realm realm) throws NodeProcessException {
        this.config = config;
        this.realm = realm;
    }
    ...
    private boolean isMigrationFlag(AMIdentity userIdentity) {
        try {
